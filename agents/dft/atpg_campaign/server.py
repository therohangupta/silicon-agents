"""HTTP service entrypoint for the ATPG Campaign agent (atpg_campaign).
...
process for atpg_campaign: it puts this agent directory on ``sys.path``, loads
``AtpgCampaignAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8227 (see Dockerfile ``EXPOSE`` and
...
from agent import AtpgCampaignAgent  # Concrete EdaAgent subclass for atpg_campaign.
...
server = AgentService.from_agent(
    ...
    AtpgCampaignAgent,  # ATPG Campaign implementation loaded from agent.py.
)
