"""HTTP service entrypoint for the DFT Validator agent (dft_validator).
...
process for dft_validator: it puts this agent directory on ``sys.path``, loads
``DftValidatorAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8231 (see Dockerfile ``EXPOSE`` and
...
from agent import DftValidatorAgent  # Concrete EdaAgent subclass for dft_validator.
...
server = AgentService.from_agent(
    ...
    DftValidatorAgent,  # DFT Validator implementation loaded from agent.py.
)
