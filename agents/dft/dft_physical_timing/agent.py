...
Hands scan ordering, test-mode SDC (clocks, false paths, case analysis), congestion, and test-mode timing/power into physical design without editing the canonical PD database unchecked.

This module defines the concrete ``DftPhysicalTimingAgent`` subclass of ``EdaAgent``.
...
Original short description preserved for continuity: DFT Physical/Timing Coordination Agent.
"""
...
class DftPhysicalTimingAgent(EdaAgent):
    """DFT Physical/Timing specialist bound to this directory's fleet manifest.
    ...
    Purpose:
        ...
        boundaries for dft_physical_timing.
