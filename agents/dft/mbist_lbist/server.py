"""HTTP service entrypoint for the MBIST/LBIST agent (mbist_lbist).
...
process for mbist_lbist: it puts this agent directory on ``sys.path``, loads
``MbistLbistAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8229 (see Dockerfile ``EXPOSE`` and
...
from agent import MbistLbistAgent  # Concrete EdaAgent subclass for mbist_lbist.
...
server = AgentService.from_agent(
    ...
    MbistLbistAgent,  # MBIST/LBIST implementation loaded from agent.py.
)
