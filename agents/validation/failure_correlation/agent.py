...
Correlates silicon symptoms with simulation, formal, signoff, firmware, and manufacturing evidence and names the earliest abstraction that reproduces the fail.

This module defines the concrete ``FailureCorrelationAgent`` subclass of ``EdaAgent``.
...
Original short description preserved for continuity: Failure-Correlation Agent.
"""
...
class FailureCorrelationAgent(EdaAgent):
    """Failure Correlation specialist bound to this directory's fleet manifest.
    ...
    Purpose:
        ...
        boundaries for failure_correlation.
