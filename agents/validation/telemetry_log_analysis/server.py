"""HTTP service entrypoint for the Telemetry / Log Analysis agent (telemetry_log_analysis).
...
process for telemetry_log_analysis: it puts this agent directory on ``sys.path``, loads
``TelemetryLogAnalysisAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
named ``agent`` that might appear later on the path. The validation meaning of
...
Default container port for this agent is 8267 (see Dockerfile ``EXPOSE`` and
...
from agent import TelemetryLogAnalysisAgent  # Concrete EdaAgent subclass for telemetry_log_analysis.
...
server = AgentService.from_agent(
    ...
    TelemetryLogAnalysisAgent,  # Telemetry / Log Analysis implementation loaded from agent.py.
)
