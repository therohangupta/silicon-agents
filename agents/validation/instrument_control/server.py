"""HTTP service entrypoint for the Instrument Control agent (instrument_control).
...
process for instrument_control: it puts this agent directory on ``sys.path``, loads
``InstrumentControlAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8265 (see Dockerfile ``EXPOSE`` and
...
from agent import InstrumentControlAgent  # Concrete EdaAgent subclass for instrument_control.
...
server = AgentService.from_agent(
    ...
    InstrumentControlAgent,  # Instrument Control implementation loaded from agent.py.
)
