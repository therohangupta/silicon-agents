"""HTTP entrypoint for the CTS Agent.

Wires ``CtsAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8247 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    CtsAgent,  # handler class for this physical-design stage
)
