"""HTTP entrypoint for the Antenna and Manufacturability Agent.

Wires ``AntennaManufacturabilityAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8253 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    AntennaManufacturabilityAgent,  # handler class for this physical-design stage
)
