"""HTTP entrypoint for the Routing Lead.

Wires ``RoutingLeadAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8249 (see connection.port in the YAML). Running this
...
from agent import RoutingLeadAgent
...
server = AgentService.from_agent(
    ...
    RoutingLeadAgent,  # handler class for this physical-design stage
)
