"""HTTP entrypoint for the SI/Noise Repair Agent.

Wires ``SiNoiseRepairAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8252 (see connection.port in the YAML). Running this
...
from agent import SiNoiseRepairAgent
...
server = AgentService.from_agent(
    ...
    SiNoiseRepairAgent,  # handler class for this physical-design stage
)
