"""HTTP entrypoint for the Global-Routing Experiment Agent.

Wires ``GlobalRoutingAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8250 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    GlobalRoutingAgent,  # handler class for this physical-design stage
)
