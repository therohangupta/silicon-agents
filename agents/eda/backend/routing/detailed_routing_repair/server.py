"""HTTP entrypoint for the Detailed-Routing Repair Agent.

Wires ``DetailedRoutingRepairAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8251 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    DetailedRoutingRepairAgent,  # handler class for this physical-design stage
)
