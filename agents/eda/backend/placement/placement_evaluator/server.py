"""HTTP entrypoint for the Independent Multi-Corner Placement Evaluator.

Wires ``PlacementEvaluatorAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8245 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    PlacementEvaluatorAgent,  # handler class for this physical-design stage
)
