"""HTTP entrypoint for the Placement Lead.

Wires ``PlacementLeadAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8243 (see connection.port in the YAML). Running this
...
from agent import PlacementLeadAgent
...
server = AgentService.from_agent(
    ...
    PlacementLeadAgent,  # handler class for this physical-design stage
)
