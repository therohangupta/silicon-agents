"""HTTP entrypoint for the Cross-Partition Boundary Coordinator.

Wires ``BoundaryCoordinatorAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8246 (see connection.port in the YAML). Running this
...
from agent import BoundaryCoordinatorAgent
...
server = AgentService.from_agent(
    ...
    BoundaryCoordinatorAgent,  # handler class for this physical-design stage
)
