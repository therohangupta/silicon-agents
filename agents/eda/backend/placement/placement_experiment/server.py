"""HTTP entrypoint for the Placement Experiment Worker.

Wires ``PlacementExperimentAgent`` into ``domains.eda.runtime.AgentService`` using this directory's
``config.yaml``. The resulting FastAPI ``app`` exposes health and task-execute
endpoints on port 8244 (see connection.port in the YAML). Running this
...
server = AgentService.from_agent(
    ...
    PlacementExperimentAgent,  # handler class for this physical-design stage
)
