"""Synthesis Lead agent module.

This module defines the thin EDAAgent subclass that represents the
Synthesis Lead service inside the backend synthesis stage of the EDA agent fleet.

In silicon design terms, this agent focuses on: RTL-to-gate synthesis orchestration: constraints, experiment hypotheses, retiming studies, logical/sequential equivalence, and netlist structural quality. PPA (power, performance, area) and equivalence acceptance decide which netlist may advance; failed equivalence always makes a candidate ineligible.

Boot wiring (see also server.py and README.md):
1. server.py constructs AgentService.from_agent(config.yaml, SynthesisLeadAgent).
2. SynthesisLeadAgent.spec is loaded from config.yaml via EDAAgent.load_config on this directory.
...
As lead, this process plans child workflows rather than running Yosys itself. Recommendations never promote onto the canonical baseline.
...
class SynthesisLeadAgent(EDAAgent):
    """Concrete fleet agent for Synthesis Lead.
