"""HTTP entrypoint for the Synthesis Lead agent process.
...
2. Import ``SynthesisLeadAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, SynthesisLeadAgent)``, which
...
EDA meaning: this process serves RTL-to-gate synthesis orchestration: constraints, experiment hypotheses, retiming studies, logical/sequential equivalence, and netlist structural quality. PPA (power, performance, area) and equivalence acceptance decide which netlist may advance; failed equivalence always makes a candidate ineligible.

The default listen port from config.yaml is 8232. Until ``EDA_FRAMEWORK`` binds an
...
Failures:
    ...
    Port bind errors occur if 8232 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Synthesis Lead; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with SynthesisLeadAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8232, context policy).
