"""Synthesis Experiment Worker agent module.

This module defines the thin EDAAgent subclass that represents the
Synthesis Experiment Worker service inside the backend synthesis stage of the EDA agent fleet.

In silicon design terms, this agent focuses on: One isolated RTL-to-netlist compile with a named hypothesis (script, flatten vs preserve hierarchy, library set). Adapter targets Yosys synth first; a licensed synthesizer later binds the same operations. Emits QoR proxies (area, cell counts, timing estimate), warnings (blackboxes, latches, loops), and a gate-level netlist.

Boot wiring (see also server.py and README.md):
1. server.py constructs AgentService.from_agent(config.yaml, SynthesisExperimentAgent).
2. SynthesisExperimentAgent.spec is loaded from config.yaml via EDAAgent.load_config on this directory.
...
class SynthesisExperimentAgent(EDAAgent):
    """Concrete fleet agent for Synthesis Experiment Worker.
