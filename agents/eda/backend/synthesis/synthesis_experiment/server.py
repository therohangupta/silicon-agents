"""HTTP entrypoint for the Synthesis Experiment Worker agent process.
...
2. Import ``SynthesisExperimentAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, SynthesisExperimentAgent)``, which
...
Failures:
    ...
    Port bind errors occur if 8234 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Synthesis Experiment Worker; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with SynthesisExperimentAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8234, context policy).
    _AGENT_DIR / "config.yaml",
    # Handler class whose tools.py callables implement the skills listed in YAML.
    SynthesisExperimentAgent,
)
