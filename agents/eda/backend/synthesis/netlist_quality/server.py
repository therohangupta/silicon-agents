"""HTTP entrypoint for the Netlist Quality agent process.
...
2. Import ``NetlistQualityAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, NetlistQualityAgent)``, which
...
EDA meaning: this process serves Structural netlist audit: combinational loops, undriven/unused nets, high fanout, unmapped or dont-use cells, DFT/scan survival, congestion-risk cells, and QoR regressions versus a recorded baseline. Catches defects synthesis QoR can hide.

The default listen port from config.yaml is 8237. Until ``EDA_FRAMEWORK`` binds an
...
Failures:
    ...
    Port bind errors occur if 8237 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Netlist Quality; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with NetlistQualityAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8237, context policy).
