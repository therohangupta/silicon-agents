"""HTTP entrypoint for the Early Congestion and Timing Evaluator agent process.
...
2. Import ``EarlyCongestionTimingAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, EarlyCongestionTimingAgent)``, which
...
EDA meaning: this process serves Pre-detail estimates of routability and timing via trial global placement and global route (OpenROAD). Identifies congestion hotspots and critical interfaces. Numbers are estimates, not signoff STA or detailed routing.

The default listen port from config.yaml is 8242. Until ``EDA_FRAMEWORK`` binds an
...
Failures:
    ...
    Port bind errors occur if 8242 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Early Congestion and Timing Evaluator; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with EarlyCongestionTimingAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8242, context policy).
    _AGENT_DIR / "config.yaml",
    # Handler class whose tools.py callables implement the skills listed in YAML.
    EarlyCongestionTimingAgent,
)
