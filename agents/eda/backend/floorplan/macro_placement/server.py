"""HTTP entrypoint for the Macro-Placement Experiment Worker agent process.
...
2. Import ``MacroPlacementAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, MacroPlacementAgent)``, which
...
EDA meaning: this process serves Hard-macro placement under halo, channel, orientation, and keepout constraints. Adapter targets OpenROAD initialize_floorplan and macro placement. Scores flyline connectivity and writes a floorplan DEF for one isolated candidate.

The default listen port from config.yaml is 8239. Until ``EDA_FRAMEWORK`` binds an
...
Failures:
    ...
    Port bind errors occur if 8239 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Macro-Placement Experiment Worker; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with MacroPlacementAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8239, context policy).
