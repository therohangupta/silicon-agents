"""HTTP entrypoint for the Floorplanning Lead agent process.
...
2. Import ``FloorplanningLeadAgent`` from ``agent.py`` (loads ``config.yaml`` into ``eda_config``).
3. Build ``AgentService`` via ``AgentService.from_agent(config.yaml, FloorplanningLeadAgent)``, which
...
EDA meaning: this process serves Partition floorplanning: macros, pin assignment, power distribution network (PDN), blockages/utilization, and early routability/timing estimates. Recommends a candidate for placement without promoting it onto the baseline.

The default listen port from config.yaml is 8238. Until ``EDA_FRAMEWORK`` binds an
...
Failures:
    ...
    Port bind errors occur if 8238 is already taken on the host.
"""
...
# Concrete EDAAgent subclass for Floorplanning Lead; importing loads config.yaml into spec.
...
# Construct the service by pairing this directory's config.yaml with FloorplanningLeadAgent.
server = AgentService.from_agent(
    # Agent manifest path (skills, boundary, connection.port=8238, context policy).
    _AGENT_DIR / "config.yaml",
    # Handler class whose tools.py callables implement the skills listed in YAML.
    FloorplanningLeadAgent,
)
