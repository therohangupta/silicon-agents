"""HTTP service entrypoint for the DFT Lead agent (dft_lead).
...
process for dft_lead: it puts this agent directory on ``sys.path``, loads
``DftLeadAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8225 (see Dockerfile ``EXPOSE`` and
...
from agent import DftLeadAgent  # Concrete EDAAgent subclass for dft_lead.
...
server = AgentService.from_agent(
    ...
    DftLeadAgent,  # DFT Lead implementation loaded from agent.py.
)
