"""HTTP service entrypoint for the DFT Physical/Timing agent (dft_physical_timing).
...
process for dft_physical_timing: it puts this agent directory on ``sys.path``, loads
``DftPhysicalTimingAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8230 (see Dockerfile ``EXPOSE`` and
...
from agent import DftPhysicalTimingAgent  # Concrete EDAAgent subclass for dft_physical_timing.
...
server = AgentService.from_agent(
    ...
    DftPhysicalTimingAgent,  # DFT Physical/Timing implementation loaded from agent.py.
)
