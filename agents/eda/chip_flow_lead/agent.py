...
Owns the program plan from design intent through release: milestone gates, cross-stage finding routing, stage replans, budget checks, and human decisions for baseline promotion and waivers. Does not approve waivers itself.

This module defines the concrete ``ChipFlowLeadAgent`` subclass of ``EDAAgent``.
...
In the chip_flow track, planners open tasks against this agent via the HTTP
...
Original short description preserved for continuity: Chip Flow Lead.
"""
...
class ChipFlowLeadAgent(EDAAgent):
    """Chip Flow Lead specialist bound to this directory's fleet manifest.
