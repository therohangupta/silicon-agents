...
Normalizes firmware logs, counters, traces, and instrument captures; correlates and summarizes sessions while keeping every claim linked to primary evidence.

This module defines the concrete ``TelemetryLogAnalysisAgent`` subclass of ``EDAAgent``.
...
Original short description preserved for continuity: Telemetry and Log-Analysis Agent.
"""
...
class TelemetryLogAnalysisAgent(EDAAgent):
    """Telemetry / Log Analysis specialist bound to this directory's fleet manifest.
    ...
    Purpose:
        ...
        boundaries for telemetry_log_analysis.
