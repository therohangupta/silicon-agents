"""HTTP service entrypoint for the Characterization agent (characterization).
...
process for characterization: it puts this agent directory on ``sys.path``, loads
``CharacterizationAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8268 (see Dockerfile ``EXPOSE`` and
...
from agent import CharacterizationAgent  # Concrete EDAAgent subclass for characterization.
...
server = AgentService.from_agent(
    ...
    CharacterizationAgent,  # Characterization implementation loaded from agent.py.
)
