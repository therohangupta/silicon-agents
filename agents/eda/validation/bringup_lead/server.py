"""HTTP service entrypoint for the Bring-up Lead agent (bringup_lead).
...
process for bringup_lead: it puts this agent directory on ``sys.path``, loads
``BringupLeadAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8263 (see Dockerfile ``EXPOSE`` and
...
from agent import BringupLeadAgent  # Concrete EDAAgent subclass for bringup_lead.
...
server = AgentService.from_agent(
    ...
    BringupLeadAgent,  # Bring-up Lead implementation loaded from agent.py.
)
