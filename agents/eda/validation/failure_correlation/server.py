"""HTTP service entrypoint for the Failure Correlation agent (failure_correlation).
...
process for failure_correlation: it puts this agent directory on ``sys.path``, loads
``FailureCorrelationAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8269 (see Dockerfile ``EXPOSE`` and
...
from agent import FailureCorrelationAgent  # Concrete EDAAgent subclass for failure_correlation.
...
server = AgentService.from_agent(
    ...
    FailureCorrelationAgent,  # Failure Correlation implementation loaded from agent.py.
)
