"""HTTP service entrypoint for the Reproduction Agent.
...
that process for ``reproduction``: it puts this agent directory on ``sys.path``, loads
``ReproductionAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import ReproductionAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8223, boundaries, observability.
    ReproductionAgent,  # Concrete EDAAgent subclass for reproduction.
)
