"""HTTP service entrypoint for the Verification Lead.
...
that process for ``verification_lead``: it puts this agent directory on ``sys.path``, loads
``VerificationLeadAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import VerificationLeadAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8215, boundaries, observability.
    VerificationLeadAgent,  # Concrete EDAAgent subclass for verification_lead.
)
