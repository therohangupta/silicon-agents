"""HTTP service entrypoint for the Failure-Triage Agent.
...
that process for ``failure_triage``: it puts this agent directory on ``sys.path``, loads
``FailureTriageAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import FailureTriageAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8222, boundaries, observability.
    FailureTriageAgent,  # Concrete EDAAgent subclass for failure_triage.
)
