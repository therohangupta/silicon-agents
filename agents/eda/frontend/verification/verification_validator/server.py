"""HTTP service entrypoint for the Verification Validator.
...
that process for ``verification_validator``: it puts this agent directory on ``sys.path``, loads
``VerificationValidatorAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import VerificationValidatorAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8224, boundaries, observability.
    VerificationValidatorAgent,  # Concrete EDAAgent subclass for verification_validator.
)
