"""HTTP service entrypoint for the Stimulus Agent.
...
that process for ``stimulus``: it puts this agent directory on ``sys.path``, loads
``StimulusAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import StimulusAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8219, boundaries, observability.
    StimulusAgent,  # Concrete EDAAgent subclass for stimulus.
)
