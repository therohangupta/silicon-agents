"""HTTP service entrypoint for the Reference Model Agent.
...
that process for ``reference_model``: it puts this agent directory on ``sys.path``, loads
``ReferenceModelAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import ReferenceModelAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8217, boundaries, observability.
    ReferenceModelAgent,  # Concrete EDAAgent subclass for reference_model.
)
