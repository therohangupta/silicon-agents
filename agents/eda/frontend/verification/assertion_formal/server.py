"""HTTP service entrypoint for the Assertion/Formal Agent.
...
that process for ``assertion_formal``: it puts this agent directory on ``sys.path``, loads
``AssertionFormalAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import AssertionFormalAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8218, boundaries, observability.
    AssertionFormalAgent,  # Concrete EDAAgent subclass for assertion_formal.
)
