...
This module defines the thin `ArchitectureLeadAgent` wrapper used by the agent fleet runtime.
...
class ArchitectureLeadAgent(EDAAgent):
    """Fleet agent for `architecture_lead` (Architecture Lead).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `architecture`, role `lead`, connection port `8203`, and the
