...
This module defines the thin `RequirementsAgent` wrapper used by the agent fleet runtime.
...
class RequirementsAgent(EDAAgent):
    """Fleet agent for `requirements` (Requirements Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `architecture`, role `worker`, connection port `8202`, and the
