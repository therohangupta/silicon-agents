"""RTL Lead — agent class entry point.

This module defines the thin `RtlLeadAgent` wrapper used by the agent fleet runtime.
...
class RtlLeadAgent(EDAAgent):
    """Fleet agent for `rtl_lead` (RTL Lead).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `rtl`, role `lead`, connection port `8208`, and the
