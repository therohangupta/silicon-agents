"""CDC/RDC Agent — agent class entry point.

This module defines the thin `CdcRdcAgent` wrapper used by the agent fleet runtime.
...
class CdcRdcAgent(EDAAgent):
    """Fleet agent for `cdc_rdc` (CDC/RDC Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `rtl`, role `worker`, connection port `8211`, and the
