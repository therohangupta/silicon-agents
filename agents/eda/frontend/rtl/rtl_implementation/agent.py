...
This module defines the thin `RtlImplementationAgent` wrapper used by the agent fleet runtime.
...
class RtlImplementationAgent(EDAAgent):
    """Fleet agent for `rtl_implementation` (RTL Implementation Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `rtl`, role `worker`, connection port `8209`, and the
