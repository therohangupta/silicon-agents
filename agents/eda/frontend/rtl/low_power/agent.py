...
This module defines the thin `LowPowerAgent` wrapper used by the agent fleet runtime.
...
class LowPowerAgent(EDAAgent):
    """Fleet agent for `low_power` (Low-Power Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `rtl`, role `worker`, connection port `8212`, and the
