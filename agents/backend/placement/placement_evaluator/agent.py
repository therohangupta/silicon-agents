...
boundaries, skills, and connection port (8245) are authoritative.
...
class PlacementEvaluatorAgent(EdaAgent):
    """Concrete fleet agent for: Re-check placement across modes/corners; reject gains that only shift violations.
    ...
    Attributes:
        spec: Loaded from sibling ``config.yaml`` (role=worker, stage=placement,
            HTTP port=8245). Skills listed there map to callables in ``tools.py``.
