"""HTTP entrypoint for the Power-Analysis Agent.

This module wires ``PowerAnalysisAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: vector-based and vectorless power estimation. Listening port (see ``config.yaml`` / Dockerfile): 8257.
"""
...
from agent import PowerAnalysisAgent  # Power-Analysis Agent class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8257, skills, boundaries for power_analysis.
    PowerAnalysisAgent,  # Agent class whose tools implement dynamic/leakage/clock power vs block budget, activity uncertainty.
)
