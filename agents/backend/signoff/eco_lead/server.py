"""HTTP entrypoint for the ECO Lead.

This module wires ``EcoLeadAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: engineering change orders (ECO) for late RTL/timing/power/physical fixes. Listening port (see ``config.yaml`` / Dockerfile): 8261.
"""
...
from agent import EcoLeadAgent  # ECO Lead class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8261, skills, boundaries for eco_lead.
    EcoLeadAgent,  # Agent class whose tools implement ECO scope, equivalence, impacted STA/DRC/LVS gates, human authorization.
)
