"""HTTP entrypoint for the DRC/LVS/ERC/DFM Agent.

This module wires ``DrcLvsAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: physical verification (DRC, LVS, ERC, density, DFM). Listening port (see ``config.yaml`` / Dockerfile): 8260.
"""
...
from agent import DrcLvsAgent  # DRC/LVS/ERC/DFM Agent class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8260, skills, boundaries for drc_lvs.
    DrcLvsAgent,  # Agent class whose tools implement DRC/LVS markers, rule decks, shorts/opens, manufacturability.
)
