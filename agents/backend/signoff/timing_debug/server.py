"""HTTP entrypoint for the Timing-Debug Agent.

This module wires ``TimingDebugAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: STA query and path classification (setup/hold/recovery/removal/clock). Listening port (see ``config.yaml`` / Dockerfile): 8256.
"""
...
from agent import TimingDebugAgent  # Timing-Debug Agent class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8256, skills, boundaries for timing_debug.
    TimingDebugAgent,  # Agent class whose tools implement OpenSTA report_checks, WNS/TNS, path cause tracing, unconstrained endpoints.
)
