"""HTTP entrypoint for the IR/EM Agent.

This module wires ``IrEmAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: IR drop and electromigration (EM) on the power grid. Listening port (see ``config.yaml`` / Dockerfile): 8258.
"""
...
from agent import IrEmAgent  # IR/EM Agent class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8258, skills, boundaries for ir_em.
    IrEmAgent,  # Agent class whose tools implement static/dynamic IR maps, EM current density, grid and placement repair.
)
