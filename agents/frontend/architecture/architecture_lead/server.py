"""HTTP entrypoint for the Architecture Lead (`architecture_lead`).
...
an `AgentService` from `config.yaml` plus `ArchitectureLeadAgent`, and exposes ASGI `app` for uvicorn.

In EDA terms this is how the **architecture** specialist becomes reachable on port 8203 for
...
from agent import ArchitectureLeadAgent  # Local agent class bound to this directory's config.yaml.
...
server = AgentService.from_agent(
    ...
    ArchitectureLeadAgent,  # Agent class whose spec and tools back task execution.
)
app = server.app  # ASGI application object for uvicorn or AgentService.run().

if __name__ == "__main__":
    # Foreground server used by Dockerfile CMD and local debugging on port 8203.
    server.run()
