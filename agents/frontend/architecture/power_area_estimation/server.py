"""HTTP entrypoint for the Power/Area Estimation Agent (`power_area_estimation`).
...
an `AgentService` from `config.yaml` plus `PowerAreaEstimationAgent`, and exposes ASGI `app` for uvicorn.

In EDA terms this is how the **architecture** specialist becomes reachable on port 8206 for
...
from agent import PowerAreaEstimationAgent  # Local agent class bound to this directory's config.yaml.
from domains.eda.server import AgentService  # Shared FastAPI/uvicorn wrapper for EDA agents.
...
server = AgentService.from_agent(
    ...
    PowerAreaEstimationAgent,  # Agent class whose spec and tools back task execution.
)
app = server.app  # ASGI application object for uvicorn or AgentService.run().

if __name__ == "__main__":
    # Foreground server used by Dockerfile CMD and local debugging on port 8206.
    server.run()
