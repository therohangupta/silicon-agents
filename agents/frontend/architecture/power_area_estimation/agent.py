...
This module defines the thin `PowerAreaEstimationAgent` wrapper used by the agent fleet runtime.
...
class PowerAreaEstimationAgent(EdaAgent):
    """Fleet agent for `power_area_estimation` (Power/Area Estimation Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `architecture`, role `worker`, connection port `8206`, and the
