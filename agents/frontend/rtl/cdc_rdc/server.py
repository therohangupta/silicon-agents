"""HTTP entrypoint for the CDC/RDC Agent (`cdc_rdc`).
...
an `AgentService` from `config.yaml` plus `CdcRdcAgent`, and exposes ASGI `app` for uvicorn.

In EDA terms this is how the **rtl** specialist becomes reachable on port 8211 for
...
from agent import CdcRdcAgent  # Local agent class bound to this directory's config.yaml.
...
server = AgentService.from_agent(
    ...
    CdcRdcAgent,  # Agent class whose spec and tools back task execution.
)
app = server.app  # ASGI application object for uvicorn or AgentService.run().

if __name__ == "__main__":
    # Foreground server used by Dockerfile CMD and local debugging on port 8211.
    server.run()
