"""HTTP entrypoint for the RTL Integration Agent (`rtl_integration`).
...
an `AgentService` from `config.yaml` plus `RtlIntegrationAgent`, and exposes ASGI `app` for uvicorn.

In EDA terms this is how the **rtl** specialist becomes reachable on port 8214 for
...
from agent import RtlIntegrationAgent  # Local agent class bound to this directory's config.yaml.
...
server = AgentService.from_agent(
    ...
    RtlIntegrationAgent,  # Agent class whose spec and tools back task execution.
)
app = server.app  # ASGI application object for uvicorn or AgentService.run().

if __name__ == "__main__":
    # Foreground server used by Dockerfile CMD and local debugging on port 8214.
    server.run()
