/**
 * @fileoverview Wide modal showing one agent's task queue plus live telemetry
 * (video, joints, action commands) during plan execution.
 */

// Status / section icons from Lucide.
import {
  CheckCircle,
  Clock,
  AlertCircle,
...
// Local UI state for expanded task result rows.
import { useState } from 'react'
// Shared modal chrome.
import { Modal } from '../common/Modal'
// Tonal status pill.
import { StatusBadge } from '../common/StatusBadge'
// Live WebSocket video canvas.
import { LiveVideoCanvas } from '../common/LiveVideoCanvas'
// Dependency id chips with hover tooltips.
import { TaskIdChip } from './TaskIdChip'
// Class merge helper.
import { cn } from '../../lib/utils'
// Live joint / action / vision stream hook.
import { useTelemetryStream } from '../../hooks/useTelemetryStream'
// Task domain type.
import type { Task } from '../../types'

/**
 * Props for {@link AgentExecutionModal}.
 */
interface AgentExecutionModalProps {
  /** Whether the modal is visible. */
  isOpen: boolean
  /** Close handler (backdrop / Escape / X). */
  onClose: () => void
  /** Agent id shown in the title. */
  agentId: string
  /** Agent type shown in the title. */
  agentType: string
  /** Tasks assigned to this agent (subset of the plan). */
  tasks: Task[]
  /** Full plan task list (for dependency lookups). */
  allTasks: Task[]
  /** Map of task_id → elapsed ms for in-progress tasks. */
  elapsedTimes: Map<number, number>
  /** Optional task-server endpoint used as telemetry key (`host:port`). */
  taskServerInfo?: { host: string; port: number } | null
}

/** Status → icon node for queue rows. */
...
/**
 * Format elapsed milliseconds as `Xs` or `Ym Zs`.
 * @param ms - Elapsed duration in milliseconds.
...
/**
 * Agent-centric execution inspector: task columns + live telemetry column.
 *
 * @param props - AgentExecutionModal props.
 * @returns Wide Modal with two-column layout.
 */
export function AgentExecutionModal({
  ...
  // Which completed tasks have their result panel expanded.
  const [expandedResults, setExpandedResults] = useState<Set<number>>(new Set())
  // Prefer host:port for telemetry when task server info is known.
  const telemetryAgentId = taskServerInfo
    ? `${taskServerInfo.host}:${taskServerInfo.port}`
    : agentId
  // Only stream while the modal is open to avoid idle sockets.
  const telemetry = useTelemetryStream(isOpen ? telemetryAgentId : null)

  // Fast lookup of any plan task by id (for dependency chips / readiness).
  const taskMap = new Map(allTasks.map(t => [t.task_id, t]))

  // Currently executing task for this agent (if any).
  const currentTask = tasks.find(t => t.status === 'in_progress')
  // Finished or failed tasks (shown in reverse chronological section).
  const completedTasks = tasks.filter(t => t.status === 'completed' || t.status === 'failed')
  // Still waiting to run.
  const pendingTasks = tasks.filter(t => t.status === 'pending')

  /**
   * Toggle result expansion for a completed task.
   * @param taskId - Task whose result panel should toggle.
   */
  ...
  /**
   * True when every dependency task is completed.
   * @param task - Pending task to evaluate.
   */
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`@${agentId} (${agentType})`} size="wide">
      {/* Two columns on large screens: tasks | telemetry */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.8fr] gap-6 max-h-[75vh] overflow-y-auto pr-1">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            {currentTask ? (
              // Highlighted amber card for the in-progress task.
              <div className="p-4 rounded-lg border-2 border-amber-400 bg-amber-50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {/* Elapsed timer when parent is tracking it. */}
              ...
              <div className="space-y-2">
                {pendingTasks.map(task => (
                  ...
                  >
                    ...
                    {/* Ready vs blocked based on dependency completion. */}
                    <span className={cn(
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {[...completedTasks].reverse().map(task => (
                  ...
                  >
                    {/* Clickable row toggles result when present. */}
                    <div
                    ...
                    >
                      ...
                      {/* Chevron only when a result payload exists. */}
                      {task.result && (
        ...
        <div className="space-y-4">
          ...
          {/* Video feed — MJPEG/H.264 stream from telemetry server media plane */}
            ...
            <div className="max-h-40 overflow-y-auto">
              {telemetry.actionLog.length > 0 ? (
                <div className="divide-y divide-slate-200">
                  {/* Show at most 20 newest entries in the panel. */}
