...
``agent_id``). It:
  ...
  6. On success: marks task completed, pops the agent queue, records world facts.
...
World state for agents comes from ``ExecutionContext.snapshot()`` plus an
...
# Replanner used when failure policy requests recovery planning.
...
# World-state snapshot builder for AgentTaskRequest.context.
from ..world_state import ExecutionContext
...
class Executor:
    ...
    def __init__(self, plan_id: int, db_url: Optional[str] = None, registry: Optional[AgentInstanceRegistry] = None):
        ...
        self.replan = False
        # Fallback world-fact strings when ExecutionContext has no rows yet.
    ...
    async def _start_task_inner(self, agent_id: int, task_id: int, task_description: str):
        ...
        try:
            ...
            # Build world-state context from prior executions.
            ...
            if not context_snapshot.completed_tasks and self.previous_task_status_messages:
                ...
                context_snapshot = ExecutionContextSnapshot(
                    ...
                    world_facts=list(self.previous_task_status_messages),
                )
            ...
            async with self.mutex:
                if result.success and not result.replan:
                    ...
                    self.agent_to_idle_bool[agent_id] = True
                    self.agent_task_map[agent_id].pop(0)
                    # Strip a legacy success prefix when building world facts.
