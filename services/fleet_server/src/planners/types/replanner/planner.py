...
class Replanner(BasePlanner):
    ...
    async def replan(self, plan_id: int, failed_task_id: int, failure_message: str, agent_task_assignments: Dict[str, List]) -> int:
        ...
        try:
            ...
            # NOTE: user.prompt historically uses {robot_task_status_context}
            # but this call passes agent_task_status_context=... — behavior
            # preserved exactly (may KeyError unless template keys match).
            ...
            try:
                ...
                # Persist as a new plan inheriting original strategies/goals.
