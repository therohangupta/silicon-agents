...
**Pytest** suite for fleet selection, EDA domain agents, engineering memory, context assembly, task lifecycle hooks, and (optionally) live Compose mesh smoke tests. Tests assume **repo root** is on **`PYTHONPATH`** (editable install or **`conftest.py`** path setup).
