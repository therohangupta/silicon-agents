...
1. Convert the fleet request into a typed ``TaskBrief``.
...
boundary (except for programming errors such as a missing ``EDAAgentConfig``).
...
# Builds a WorkflowProposal from a lead's plan_steps or delegates_to list.
...
from ..schemas.enums import RecordType, TaskOutcome, ValidationState
...
from ..schemas.messages import WorkflowProposal
# Domain task contract and the result object returned to the fleet layer.
from ..schemas.task import TaskResult, TaskBrief
...
class EDAAgent(TaskLifecycle):
    ...
    ``eda_config: EDAAgentConfig`` (usually produced by ``load_config`` against their
    ...
    * Validator agents additionally publish a provisional gate decision that
    ...
    Failure modes: missing ``eda_config`` raises ``TypeError`` at construction.
    ...
    # Class attribute every concrete subclass must set to a loaded EDAAgentConfig.
    eda_config: EDAAgentConfig

    @classmethod
    def load_config(cls, directory: str | Path) -> EDAAgentConfig:
        """Load the ``EDAAgentConfig`` from ``config.yaml`` beside an agent directory.

        This is the usual way concrete agent modules obtain their class-level
        ``eda_config``. It delegates to ``load_eda_agent_config``, which requires a
        ...
        Returns:
            A fully populated ``EDAAgentConfig`` for that directory.

        Side effects:
            Reads ``config.yaml`` from disk. Does not mutate class state.

        Failures:
            Propagates ``EDAConfigError`` (and underlying I/O or YAML errors)
        ...
        return load_eda_agent_config(directory)
    ...
    def __init__(
        ...
        Failures:
            ...
            ``eda_config`` to an ``EDAAgentConfig`` instance before construction.
        """
        # Reject subclasses that forgot to assign a loaded EDAAgentConfig.
        if not isinstance(getattr(self, "eda_config", None), EDAAgentConfig):
            raise TypeError(f"{type(self).__name__} must set eda_config to an EDAAgentConfig")
    ...
    def decode_task(self, request: AgentTaskRequest) -> TaskBrief:
        """Decode the fleet envelope into the EDA task contract."""
        return TaskBrief.from_request(request)

    def prepare_task(self, task: TaskBrief) -> TaskBrief:
        """Apply this EDA agent's idempotency and stage defaults."""
        if not task.idempotency_key:
            task.idempotency_key = f"{self.eda_config.agent_id}:{task.task_id}"
        if not task.stage:
            task.stage = self.eda_config.stage
        return task

    async def assemble_context(self, task: TaskBrief) -> Any:
        """Resolve this EDA agent's context policy through engineering memory."""
        return await self.context.assemble(task, self.eda_config.context)

    async def journal_started(self, task: TaskBrief, context: Any) -> None:
        ...
        await self.memory.append(
            ...
            agent_id=self.eda_config.agent_id,
            summary=f"Started {self.eda_config.agent_id}",
            design_revision=task.design_revision,
        )

    async def execute_task(self, task: TaskBrief, context: Any) -> TaskResult:
        """Run the EDA role-specific execution path with its context."""
        return await self.act(task, context)

    async def journal_finished(self, task: TaskBrief, result: TaskResult) -> None:
    ...
    def failure_result(self, task: TaskBrief, exc: Exception) -> TaskResult:
    ...
    async def act(self, task: TaskBrief, context: Any) -> TaskResult:
        """Dispatch to planning or tool execution based on the agent's role.

        Args:
            task: Fully prepared ``TaskBrief`` for this invocation.
        ...
        if self.eda_config.role == EDAAgentRole.LEAD:
    ...
    async def plan(self, task: TaskBrief, context: Any) -> TaskResult:
        """Propose a child workflow and optionally invoke unbound lead tools.

        Builds a ``WorkflowProposal`` from explicit ``plan_steps`` or from
        ...
        await self.memory.publish(
            ...
            summary=f"{self.eda_config.display_name} proposed {len(workflow.tasks)} task(s)",
        ...
        return TaskResult(
            ...
            summary=(
                f"{self.eda_config.display_name} proposed a {len(workflow.tasks)}-task workflow "
    ...
    async def execute_tools(self, task: TaskBrief, context: Any) -> TaskResult:
        ...
        if self.eda_config.role == EDAAgentRole.VALIDATOR:
        ...
        return TaskResult(
            ...
            summary=(
                f"{self.eda_config.display_name} invoked {len(observations)} tool(s). "
                "No EDA framework ran, so this is not an engineering acceptance."
            ),
            observations=observations,
        )

    async def _validator_result(self, task: TaskBrief, observations: list[dict[str, Any]]) -> TaskResult:
        ...
        gate = await self.memory.publish(
            ...
            payload={
                "passed": False,
                "candidate": candidate,
                "validator": self.eda_config.agent_id,
                "observations": observations,
            },
        ...
        return TaskResult(
            ...
            reason_code="FRAMEWORK_UNBOUND",
            summary=(
                f"{self.eda_config.display_name} recorded a provisional gate. "
    ...
    async def _invoke_declared_tools(self, task: TaskBrief, context: Any) -> list[dict[str, Any]]:
        """Permission-check and call every tool listed on ``self.eda_config.tools``.
        ...
        # Walk the EDA skill-permission declarations from config.yaml.
        for tool_spec in self.eda_config.skills:
            # Enforce role matrix and task allow/forbid before any side effects.
            self.assert_action(tool_spec.operation, task)
    ...
    def assert_action(self, action: EDAOperation, task: TaskBrief) -> None:
        ...
        Args:
            action: The ``EDAOperation`` declared on an ``EDAAgentSkill``.
            task: The current ``TaskBrief`` carrying allow/forbid lists.
        ...
        if action.value in task.forbidden_actions:
            raise AgentPermissionError(f"{self.eda_config.agent_id} cannot {action.value} on this task")
        ...
        if action not in ROLE_OPERATIONS[self.eda_config.role]:
            raise AgentPermissionError(f"{self.eda_config.role.value} agents cannot {action.value}")

    def _scope(self, task: TaskBrief) -> MemoryScope:
        ...
        return MemoryScope(
            ...
            block=task.block,
            stage=task.stage or self.eda_config.stage,
        )

    def _failure(self, task: TaskBrief, outcome: TaskOutcome, reason: str, summary: str) -> AgentTaskResult:
...
def workflow_from_result(result: AgentTaskResult) -> Optional[WorkflowProposal]:
    """Extract a typed ``WorkflowProposal`` from a fleet task result's artifacts.
    ...
    Returns:
        A validated ``WorkflowProposal`` when the artifact is a dict shaped like
    ...
    return WorkflowProposal.model_validate(raw)
