"""
Per-plan execution context reconstruction for agent task payloads.
...
completed task summaries, free-form execution facts (result messages), and
...
``packages.fleet_sdk.src.executor.executor.Executor._start_task_inner``.
"""
...
class ExecutionContext:
    """
    Structured per-plan execution context reconstructed from task executions.
    ...
    def __init__(self, registry: AgentInstanceRegistry, plan_id: int):
        ...
        Args:
            ...
            plan_id: Plan whose historical executions form the execution context.
        """
    ...
    async def snapshot(self) -> ExecutionContextSnapshot:
        ...
        a ``TaskSummary``, optionally an execution-fact string from the message,
        and any parseable ``artifact_refs`` entries.

        Returns:
            An ``ExecutionContextSnapshot`` with plan summary text, completed
            task summaries, execution facts, and available artifact refs.
        """
        ...
        for execution in executions:
            ...
            # Non-empty messages become free-form execution facts for downstream agents.
            if message:
                execution_facts.append(message)
