...
class MonolithicPlanner(BasePlanner):
    ...
    async def plan(self, goal_ids: List[int]) -> str:
        ...
        graph = DAGPlan.model_validate_json(response.choices[0].message.content)
        logger.info("Generated monolithic DAG for goals %s with %s nodes", goal_ids, len(graph.nodes))
        self.planning_artifacts = {
            "goals": [goal.description for goal in goals],
            "agent_context": agent_context,
            "capabilities": capabilities,
            "dag_structure": graph.model_dump(),
            "generated_plan": graph.model_dump_json(),
        }
        return graph
