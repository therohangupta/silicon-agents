...
|-------|------|----------------|
| **Gateway (BFF)** | `services/gateway/src/` | **8000** (`GATEWAY_PORT`) |
| **Telemetry service** | `services/telemetry/src/` | **9000** (`TELEMETRY_PORT`) |
| **Fleet server** (gRPC) | `services/fleet_server/src/` | **50051** |
| **Dashboard frontend** | `services/dashboard-web/src/` | **5173** (dev) |

Docker Compose wires `TELEMETRY_URL` on the gateway to the telemetry container and runs all three backend services together (see `docker-compose.yml`).
