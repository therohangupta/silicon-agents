...
- **Fleet control plane (gRPC server)**: `services/fleet_server/src/service.py`
  - Execution orchestration: `services/fleet_server/src/executor/executor.py`
  - DB access: `packages/fleet_sdk/src/instance_registry.py` (`AgentInstanceRegistry`; also used by gateway gRPC bridge, planners, and allocators)
  - **Fleet → gateway event callback**: `services/fleet_server/src/events.py` — fire-and-forget HTTP POST to `GATEWAY_EVENT_URL` (from `packages/config.py`: same host/port as the gateway plus `/internal/events`; Docker Compose sets this explicitly, e.g. `http://gateway:8000/internal/events`)
- **Gateway/BFF (FastAPI)**: `services/gateway/src/app.py`
  - gRPC to fleet: `services/gateway/src/grpc_bridge.py`
  - HTTP API routers: `services/gateway/src/routers/` — e.g. `plans.py`, `agents.py`, `tasks.py`, `goals.py`, `websocket.py`, `telemetry.py`, `metrics.py`, …
  - Agent health for the UI: HTTP client to the Telemetry service in `services/gateway/src/services/telemetry_client.py` (used from `routers/agents.py` via `TELEMETRY_URL` in gateway config)
- **Telemetry service (FastAPI)**, separate from gateway: `services/telemetry/src/`
...
- **Frontend (Vite/React)**: `services/dashboard-web/src/` — `pages/`, `components/`, `lib/`, …
  - Calls `/api/...` on the gateway (already aligned with “single UI boundary”)
- **Agent task servers**: `agents/` — one directory per agent, each with `config.yaml`, `server.py`, and `tools.py`
- **Shared ports/URLs**: `packages/config.py` (e.g. `DATABASE_URL`, `GATEWAY_EVENT_URL`; gateway and telemetry import their own `config` modules that align with these)
...
- On control-plane mutations, the fleet server POSTs a small JSON event to the gateway (`GATEWAY_EVENT_URL`, handled in `services/gateway/src/routers/websocket.py` as `POST /internal/events`).
...
- `control_plane/...`
- `data_plane/...` (telemetry already lives in `services/telemetry/` today)
- `storage/...` (DB models + registry)
