...
Operator and maintainer **utilities** — short-lived processes, not fleet services. Run Python entrypoints from **`agent_fleet/`** (repository path to the package root) so **`domains.*`** and **`packages.*`** import without extra **`PYTHONPATH`** hacks. Shell scripts resolve **`agent_fleet/`** relative to their own location.
...
./agent_fleet/scripts/startup.sh agent_fleet/fleets/architecture.yaml --build
```

Fleet files live under **`agent_fleet/fleets/`**; see [`../fleets/README.md`](../fleets/README.md).
...
| Python utilities | Insert **`agent_fleet/`** on **`sys.path`** when run as files |
| **`database_mgmt/*.sh`** | Expect to be run from **`agent_fleet/`** so **`docker compose`** finds the project file |
