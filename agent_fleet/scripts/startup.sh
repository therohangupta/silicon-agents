...
# Lives at agent_fleet/scripts/startup.sh.
#
# What this script starts (always, unless a step fails):
#   1. Platform Compose project (agent_fleet/docker-compose.yml): Postgres,
#      fleet-server, gateway, telemetry, NATS, memory-plane stores, etc.
#   2. Agent Compose project "silicon-agents" (agent_fleet/docker-compose.agents.yml),
...
#   ./agent_fleet/scripts/startup.sh agent_fleet/fleets/architecture.yaml
...
#
# The YAML lists agent directories under agents:. See agent_fleet/fleets/.
# =============================================================================
...
# This file lives at agent_fleet/scripts/startup.sh. APP is agent_fleet/;
# ROOT is the repo root (parent of APP). Relative fleet paths resolve under ROOT.
...
if [[ -z "${FLEET}" || "${FLEET}" == "-h" || "${FLEET}" == "--help" ]]; then
  echo "usage: ./agent_fleet/scripts/startup.sh <fleet.yaml> [--build]" >&2
  exit 2
fi
...
# All subsequent relative paths / docker compose defaults are relative to agent_fleet/.
cd "${APP}"
