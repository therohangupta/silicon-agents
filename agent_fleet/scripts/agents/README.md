...
These shell scripts previously built and ran **physical demo** agent containers that lived outside the current EDA catalog under **`agent_fleet/agents/`**. Those packages were removed during the silicon catalog migration.
...
1. Pick an agent directory, e.g. **`agent_fleet/agents/frontend/architecture/requirements/`**
...
```bash
# From agent_fleet/ after editable install
