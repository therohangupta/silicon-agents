"""HTTP service entrypoint for the Lab Procedure agent (lab_procedure).
...
process for lab_procedure: it puts this agent directory on ``sys.path``, loads
``LabProcedureAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8264 (see Dockerfile ``EXPOSE`` and
...
from agent import LabProcedureAgent  # Concrete EdaAgent subclass for lab_procedure.
...
server = AgentService.from_agent(
    ...
    LabProcedureAgent,  # Lab Procedure implementation loaded from agent.py.
)
