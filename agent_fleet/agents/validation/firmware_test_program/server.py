"""HTTP service entrypoint for the Firmware / Test Program agent (firmware_test_program).
...
process for firmware_test_program: it puts this agent directory on ``sys.path``, loads
``FirmwareTestProgramAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8266 (see Dockerfile ``EXPOSE`` and
...
from agent import FirmwareTestProgramAgent  # Concrete EdaAgent subclass for firmware_test_program.
...
server = AgentService.from_agent(
    ...
    FirmwareTestProgramAgent,  # Firmware / Test Program implementation loaded from agent.py.
)
