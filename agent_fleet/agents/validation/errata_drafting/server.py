"""HTTP service entrypoint for the Errata Drafting agent (errata_drafting).
...
process for errata_drafting: it puts this agent directory on ``sys.path``, loads
``ErrataDraftingAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8270 (see Dockerfile ``EXPOSE`` and
...
from agent import ErrataDraftingAgent  # Concrete EdaAgent subclass for errata_drafting.
...
server = AgentService.from_agent(
    ...
    ErrataDraftingAgent,  # Errata Drafting implementation loaded from agent.py.
)
