"""HTTP entrypoint for the Thermal and Reliability Agent.

This module wires ``ThermalReliabilityAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: thermal, aging, variation, and reliability margins. Listening port (see ``config.yaml`` / Dockerfile): 8259.
"""
...
from agent import ThermalReliabilityAgent  # Thermal and Reliability Agent class with loaded signoff spec.
from domains.eda.server import AgentService  # Shared FastAPI service factory for EDA agents.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8259, skills, boundaries for thermal_reliability.
    ThermalReliabilityAgent,  # Agent class whose tools implement hotspots, aging margins, thermal feedback to leakage/delay, package assumptions.
)
