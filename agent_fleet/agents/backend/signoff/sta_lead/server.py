"""HTTP entrypoint for the STA Lead.

This module wires ``StaLeadAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: multi-mode multi-corner (MMMC) static timing ownership. Listening port (see ``config.yaml`` / Dockerfile): 8255.
"""
...
from agent import StaLeadAgent  # STA Lead class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8255, skills, boundaries for sta_lead.
    StaLeadAgent,  # Agent class whose tools implement MMMC coverage, path-group repair coordination, constraint edit requests.
)
