"""HTTP entrypoint for the Independent Signoff Validator.

This module wires ``SignoffValidatorAgent`` into the shared ``AgentService`` so the orchestrator can
...
Domain: independent signoff gate grading and waiver audit. Listening port (see ``config.yaml`` / Dockerfile): 8262.
"""
...
from agent import SignoffValidatorAgent  # Independent Signoff Validator class with loaded signoff spec.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Manifest: port 8262, skills, boundaries for signoff_validator.
    SignoffValidatorAgent,  # Agent class whose tools implement reproduce STA/DRC/LVS, audit waivers, emit pass/fail signoff gate.
)
