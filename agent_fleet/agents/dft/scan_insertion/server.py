"""HTTP service entrypoint for the Scan Insertion agent (scan_insertion).
...
process for scan_insertion: it puts this agent directory on ``sys.path``, loads
``ScanInsertionAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
Default container port for this agent is 8226 (see Dockerfile ``EXPOSE`` and
...
from agent import ScanInsertionAgent  # Concrete EdaAgent subclass for scan_insertion.
...
server = AgentService.from_agent(
    ...
    ScanInsertionAgent,  # Scan Insertion implementation loaded from agent.py.
)
