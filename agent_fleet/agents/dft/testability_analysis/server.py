"""HTTP service entrypoint for the Testability Analysis agent (testability_analysis).
...
process for testability_analysis: it puts this agent directory on ``sys.path``, loads
``TestabilityAnalysisAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
named ``agent`` that might appear later on the path. The dft meaning of
...
Default container port for this agent is 8228 (see Dockerfile ``EXPOSE`` and
...
from agent import TestabilityAnalysisAgent  # Concrete EdaAgent subclass for testability_analysis.
...
server = AgentService.from_agent(
    ...
    TestabilityAnalysisAgent,  # Testability Analysis implementation loaded from agent.py.
)
