...
This module defines the thin `SecurityReliabilityAgent` wrapper used by the agent fleet runtime.
...
class SecurityReliabilityAgent(EdaAgent):
    """Fleet agent for `security_reliability` (Security/Reliability Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `architecture`, role `worker`, connection port `8207`, and the
