...
This module defines the thin `InterfaceAgent` wrapper used by the agent fleet runtime.
...
class InterfaceAgent(EdaAgent):
    """Fleet agent for `interface` (Interface Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `architecture`, role `worker`, connection port `8205`, and the
