"""HTTP entrypoint for the Performance Modeling Agent (`performance_modeling`).
...
an `AgentService` from `config.yaml` plus `PerformanceModelingAgent`, and exposes ASGI `app` for uvicorn.

In EDA terms this is how the **architecture** specialist becomes reachable on port 8204 for
...
from agent import PerformanceModelingAgent  # Local agent class bound to this directory's config.yaml.
...
server = AgentService.from_agent(
    ...
    PerformanceModelingAgent,  # Agent class whose spec and tools back task execution.
)
app = server.app  # ASGI application object for uvicorn or AgentService.run().

if __name__ == "__main__":
    # Foreground server used by Dockerfile CMD and local debugging on port 8204.
    server.run()
