"""HTTP service entrypoint for the UVM Environment Agent.
...
that process for ``uvm_environment``: it puts this agent directory on ``sys.path``, loads
``UvmEnvironmentAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import UvmEnvironmentAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8216, boundaries, observability.
    UvmEnvironmentAgent,  # Concrete EdaAgent subclass for uvm_environment.
)
