"""HTTP service entrypoint for the Regression Agent.
...
that process for ``regression``: it puts this agent directory on ``sys.path``, loads
``RegressionAgent`` and ``config.yaml``, wraps them in ``AgentService``, and
...
from agent import RegressionAgent  # Specialist class defined in this directory's agent.py.
...
server = AgentService.from_agent(
    _AGENT_DIR / "config.yaml",  # Fleet spec: skills, port 8221, boundaries, observability.
    RegressionAgent,  # Concrete EdaAgent subclass for regression.
)
