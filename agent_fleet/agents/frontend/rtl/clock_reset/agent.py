...
This module defines the thin `ClockResetAgent` wrapper used by the agent fleet runtime.
...
class ClockResetAgent(EdaAgent):
    """Fleet agent for `clock_reset` (Clock/Reset Agent).

    Purpose:
        Provide a typed agent object whose `spec` attribute mirrors `config.yaml`, including
        stage `rtl`, role `worker`, connection port `8210`, and the
