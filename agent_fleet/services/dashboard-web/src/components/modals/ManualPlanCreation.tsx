/**
 * @fileoverview Manual plan creation modal — operator-authored task DAGs.
 *
 * Lets the user pick goals, author tasks with dependencies and optional agent
 * assignments, preview a Graphviz dependency graph, validate that every selected
 * goal is used, and POST via `plansApi.createManual`.
 */

// React namespace for refs; state for form/DAG; effect for keyboard + Graphviz.
import React, { useState, useEffect } from 'react'
// Goals/agents queries and manual plan create mutation.
import { useQuery, useMutation } from '@tanstack/react-query'
// Icons for search, selection, task CRUD, agents, and dependency affordances.
import { Search, CheckCircle, Plus, Trash2, Bot, ArrowRight } from 'lucide-react'
// Primary create dialog + nested validation error dialog.
import { Modal } from '../common/Modal'
// Goal selection tiles and task editor rows.
import { Card } from '../common/Card'
// Form actions and DAG reset control.
import { Button } from '../common/Button'
// Gateway APIs for goals, agents, and manual plan create.
import { goalsApi, agentsApi, plansApi } from '../../lib/api'
// Class merge for selected vs unselected goal cards.
import { cn } from '../../lib/utils'

/**
 * Props for {@link ManualPlanCreation}.
 */
interface ManualPlanCreationProps {
  /** Whether the create modal is visible. */
  isOpen: boolean
  /** Close handler (parent typically also refreshes plans). */
  onClose: () => void
}

/**
 * Client-side task draft before server ids exist.
 */
interface TaskData {
  /** Temporary id like `t1` used in dependency edges. */
  temp_id: string
  /** Task description text. */
  description: string
  /** Upstream temp_ids this task depends on. */
  dependencies: string[]
  /** Optional assigned agent id. */
  agent_id?: string
  /** Goal this task contributes to (required before submit). */
  goal_id?: number
}

/**
 * Wide modal wizard for manually authoring a plan DAG.
 * @param props - ManualPlanCreation props.
 */
export function ManualPlanCreation({ isOpen, onClose }: ManualPlanCreationProps) {
  // Multi-select of goals the plan should cover.
  const [selectedGoals, setSelectedGoals] = useState<number[]>([])
  // Filter string for the goal picker.
  const [goalSearch, setGoalSearch] = useState<string>('')
  // Required plan name field.
  const [planName, setPlanName] = useState<string>('')
  // Required plan description field.
  const [planDescription, setPlanDescription] = useState<string>('')
  // Validation error message shown in a nested modal (null = hidden).
  const [goalAssignmentError, setGoalAssignmentError] = useState<string | null>(null)

  // Task creation state — drafts with temp ids.
  const [tasks, setTasks] = useState<TaskData[]>([])
  // Monotonic counter for next `tN` temp id.
  const [nextTaskId, setNextTaskId] = useState<number>(1)
  // Controlled input for the "add task" row.
  const [newTaskDescription, setNewTaskDescription] = useState<string>('')

  // DAG visualization state (Graphviz SVG + pan/zoom).
  ...
  // Load all goals for the picker.
  const { data: goals = [] } = useQuery({
    queryKey: ['goals'],
    queryFn: goalsApi.list,
  })

  // Load agents for per-task allocation dropdowns.
  const { data: agents = [] } = useQuery({
    queryKey: ['agents'],
    queryFn: () => agentsApi.list(),
  })

  // Filter goals by id substring or description substring.
  ...
  /**
   * Append a new task draft defaulting goal_id to the first selected goal.
   */
  const addTask = () => {
  ...
  /**
   * Remove a task draft by temp id (does not rewrite other deps).
   * @param taskTempId - Temp id to remove.
   */
  ...
  /**
   * Patch fields on a task draft.
   * @param taskTempId - Temp id to update.
   * @param updates - Partial TaskData fields to merge.
   */
  ...
  // Keyboard shortcuts for DAG (only while modal open): r reset, +/- zoom.
  useEffect(() => {
    /**
     * Handle r / + / - while the modal is mounted open.
   ...
   * @param e - Mouse down event on the DAG viewport.
   */
   ...
   * @param e - Wheel event on the DAG viewport.
   */
  ...
  /** Reset zoom/pan to the default framing for the preview. */
  const resetView = () => {
    setZoom(0.8)
    setPan({ x: 0, y: 0 })
  }

  // Regenerate Graphviz SVG whenever the task draft list changes.
  useEffect(() => {
...
`
      ...
      tasks.forEach(task => {
        ...
        // Find agent data if assigned
        const assignedAgent = agents.find(r => r.agent_id === task.agent_id)
        const agentType = assignedAgent ? assignedAgent.agent_type : 'unassigned'
        const agentId = task.agent_id || ''

        // Calculate width for the visible content
        const badgeContentWidth = agentType.length * 16 + 24
  ...
  // POST the manual plan; close on success (list refresh left to parent/realtime).
  ...
  /**
   * Validate goal/task assignment rules then submit `createManual`.
   * @param e - Form submit event.
   */
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Hard-require goals, name, description, and at least one task.
  ...
  /**
   * Toggle a goal id in/out of the selectedGoals multi-select.
   * @param goalId - Goal id clicked in the picker grid.
   */
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Create Plan Manually" size="wide">
        ...
        {/* Goals Selection — searchable multi-select cards */}
        <div>
          ...
          {/* Goals Grid — click to toggle selection */}
        ...
        {/* Create and Allocate Tasks section */}
        <div className="border-t border-slate-200 pt-6">
          ...
          {/* Add New Task input + plus button */}
          <div className="mb-6">
              ...
              <Card key={task.temp_id} className="p-4 border-slate-200">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {/* Dependencies and Agent Allocation controls */}
                    <div className="flex items-center gap-4 flex-wrap">
                      {/* Multi-select dependencies (prior tasks only) */}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          value={task.agent_id || ''}
                          onChange={(e) => updateTask(task.temp_id, { agent_id: e.target.value || undefined })}
                          className="px-3 py-1 bg-slate-50 border border-slate-200 rounded text-[var(--color-text)] text-xs focus:outline-none focus:ring-1 focus:ring-amber-500"
                        >
                  ...
                  {/* Required goal assignment from selected goals */}
                  <div className="flex items-center gap-2">
        ...
        {/* Live Graphviz preview of task dependencies */}
        {tasks.length > 0 && (
          <div className="border-t border-slate-200 pt-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold text-[var(--color-text)] flex items-center gap-3">
      ...
      {/* Nested validation error modal for goal assignment issues */}
      <Modal
