/**
 * @fileoverview Plans list page — create, allocate, search, and launch fleet plans.
 *
 * Route: `/plans`
 *
 * A **Plan** is the central work unit of the agent fleet: it binds one or more
 * **Goals** to a **planning strategy** (which planner method decomposes goals
 * into tasks) and later an **allocation strategy** (which allocator method
 * assigns those tasks to registered **Agents**). Plans progress through
 * allocation and execution statuses that this page filters and displays.
 *
 * Operators use this page to:
 * - Browse all plans with search / status filters and sort controls
 * - Create a plan via AI planner methods OR open ManualPlanCreation
 * - Allocate an unallocated plan to agents (AllocatePlanForm)
 * - Copy / delete / navigate into PlanDetails or the Execution Monitor
 * - Open MethodDetailModal for planner/allocator YAML introspection
 *
 * Real-time updates via `useRealtimeUpdates` keep plan statuses fresh as the
 * fleet server mutates execution state (telemetry-adjacent listing refresh).
 *
...
import { plansApi, goalsApi, agentsApi, methodsApi } from '../lib/api'
...
import type { Agent, Goal, PlanStatus, PlanAllocationStatus } from '../types'
import type { MethodSummary } from '../lib/api'

/**
 * Local UI model for a planner or allocator strategy option during plan creation.
 */
interface LocalStrategy {
  /** Strategy identifier (method id or local key). */
  id: string
  /** Operator-facing strategy display name. */
  name: string
  /** Whether this option is a planning or allocation strategy. */
...
/**
 * Main Plans list page — browse, create, allocate, copy, delete, and launch plans.
 *
 * Combines React Query fetches, realtime invalidation, status filters, and
 * modals (CreatePlanForm, ManualPlanCreation, AllocatePlanForm, MethodDetailModal).
 */
export function Plans() {
  ...
  // Plan id currently being copied (loading affordance on Copy action).
  const [copyingPlanId, setCopyingPlanId] = useState<number | null>(null)
  // Client-side cache of per-plan status badges refreshed via polling/realtime.
  ...
  /**
   * Component/helper `getStatus` used by this fleet dashboard page.
   *
  ...
  // Fetch agent health statuses for accurate execution readiness
      ...
      {plans.length === 0 ? (
        allPlans.length === 0 ? (
          <EmptyState
            ...
            description="Create your first plan to define agent task execution sequences."
            action={
              <Button onClick={() => setIsModalOpen(true)}>
            ...
            if (executionStatus === 'completed') {
              ...
              try {
                if (plan.allocation_artifacts?.final_allocation?.allocations) {
                  ...
                  assignedAgentIds = allocations
                    .map((allocation: any) => allocation?.agent_id)
                    .filter((agentId: string) => agentId && agentId !== 'unassigned') || []
              ...
              if (agentsReady) {
                ...
                badgeConfig = { bg: 'bg-yellow-100', border: 'border-yellow-400', text: 'text-yellow-950', label: 'Agents Offline', icon: AlertTriangle }
              }
...
/**
 * Props for CreatePlanForm — goals, strategies, submit/cancel, loading flag.
 */
...
/**
 * Form that selects goals + a planning strategy and submits plan creation.
 *
 * Invokes the fleet planner pipeline so goals become a task DAG (plan).
 *
 * @param props.goals - Available goals to attach.
 * @param props.strategies - LocalStrategy options (planning methods).
 * @param props.onSubmit - Called with form payload for plansApi.create.
...
function CreatePlanForm({ goals, strategies, onSubmit, onCancel, isLoading }: CreatePlanFormProps) {
  // Local state/binding `[selectedGoals, setSelectedGoals]` for this fleet UI flow.
  const [selectedGoals, setSelectedGoals] = useState<number[]>([])
  // Local state/binding `[selectedPlanner, setSelectedPlanner]` for this fleet UI flow.
  ...
  // Local state/binding `[skipAllocation, setSkipAllocation]` for this fleet UI flow.
  const [skipAllocation, setSkipAllocation] = useState<boolean>(false)
  // Local state/binding `[plannerFilter, setPlannerFilter]` for this fleet UI flow.
  ...
  // Local state/binding `[goalSearch, setGoalSearch]` for this fleet UI flow.
  const [goalSearch, setGoalSearch] = useState<string>('')
  // Local state/binding `[planName, setPlanName]` for this fleet UI flow.
  const [planName, setPlanName] = useState<string>('')
  // Local state/binding `[planDescription, setPlanDescription]` for this fleet UI flow.
  ...
  // Local state/binding `filteredAllocators` for this fleet UI flow.
  ...
  // Local state/binding `allocatorMethodTypes` for this fleet UI flow.
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {filteredPlanners.map((planner) => (
                <MethodSelectionCard
                  ...
                  onClick={() => setSelectedPlanner(planner.id ?? null)}
                />
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {filteredAllocators.map((allocator) => (
                <MethodSelectionCard
                  ...
                  onClick={() => setSelectedAllocator(allocator.id ?? null)}
                />
              ))}
            </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-[var(--color-text-secondary)]">Create plan without automatic agent allocation - assign agents manually later</p>
                  </div>
                </div>
              </Card>
            </div>
