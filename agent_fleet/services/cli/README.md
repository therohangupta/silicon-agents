...
| Location | What it is |
|----------|------------|
| **`agent_fleet/cli/`** | Production **Click CLI** (`agentctl`): register agents, goals, tasks, plans via `FleetManagerClient` gRPC |
| **`services/cli/` (here)** | Reserved name for a future **image** that would run the same commands inside Docker/CI |

Operators install the fleet editable (`pip install -e .` from `agent_fleet/`) and
...
4. Keep **`agent_fleet/cli/`** as the single source of Click command definitions —
