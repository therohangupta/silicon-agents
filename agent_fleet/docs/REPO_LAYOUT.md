...
This document describes the **current** `agent_fleet/` directory layout: deployable services, shared packages, agents, and tooling. Earlier drafts proposed a different shape (`fleet-core`, nested `gateway/` packages, `tools/docker/`); that material is superseded by what is on disk now.
