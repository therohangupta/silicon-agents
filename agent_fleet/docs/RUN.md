# How to run and test agent_fleet

Everything runs from the **agent_fleet** directory (this package). No old_agent_fleet.

**Layout:** Your workspace root is the directory that **contains** the `agent_fleet/` folder. Paths below use that convention.

**Postgres:** Defaults for bare-metal Python services match `packages/config.py`: host `localhost`, port **5432**, database `agent_fleet`, user `agent_user`, password `secret`. Override with `DATABASE_URL` or `DB_*` env vars. **Docker Compose** maps the container to the host as **localhost:5433** → container `5432`, so if only the DB runs in Compose and the stack runs on the host, set `DB_PORT=5433` (or a full `DATABASE_URL` using port 5433).

Optional: for Compose, `docker-compose.yml` loads `../.env` (the directory **above** `agent_fleet/`, i.e. the same workspace root that contains `agent_fleet`) for extra env on **fleet-server**.

You need **four terminals** for bare-metal dev (or use Docker Compose). From the **workspace root** (the directory that contains `agent_fleet`):
...
```bash
cd agent_fleet/services/gateway
...
|--------------|-------------------------------------------------------------------------------------------------|-------|
| Fleet server  | `cd agent_fleet && python -m services.fleet_server.src -v`                                   | 50051 |
| Telemetry     | `cd agent_fleet && python -m services.telemetry.src --port 9000`                             | 9000  |
| Gateway       | `cd agent_fleet/services/gateway && uvicorn src.main:app --reload --port 8000`               | 8000  |
| Frontend      | `cd agent_fleet/services/dashboard-web && npm run dev`                                       | 5173  |

All four run against the **agent_fleet** tree; use `pip install -e .` from `agent_fleet/` first for Python imports and `agentctl`.

---

## Docker (scalable)

Compose files live in **`agent_fleet/`**: `docker-compose.yml` and `docker-compose.dev.yml`.
...
Run the **frontend** on the host for dev: `cd agent_fleet/services/dashboard-web && npm install && npm run dev`, then open http://localhost:5173 (proxies to the gateway on port 8000).
...
- **Fleet server**: gRPC on 50051. From the host you can use the CLI: `agentctl world list`, `agentctl agents list`, etc. (requires `pip install -e .` from `agent_fleet/`; point at `localhost:50051` or use gateway-backed flows as appropriate.)
