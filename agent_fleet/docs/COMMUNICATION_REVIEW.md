...
> **Document status (April 2026):** This file is a **living reference**. It describes the current stack and what is still worth tightening. Paths below are under the `agent_fleet/` tree unless noted.
>
...
- **Frontend:**  
  `agent_fleet/services/dashboard-web/src/main.tsx` (default query options),  
  `agent_fleet/services/dashboard-web/src/pages/Dashboard.tsx` (`agent-health` / aggregation),  
  `agent_fleet/services/dashboard-web/src/pages/Execution.tsx` (plan `refetchInterval`; task WS already wired).
- **Gateway:**  
  `agent_fleet/services/gateway/src/routers/websocket.py` (`/internal/events`, `ws/global-updates`, `ws/execution/{plan_id}`),  
  `agent_fleet/services/gateway/src/services/telemetry_client.py`,  
  `agent_fleet/services/gateway/src/app.py` / `main.py` (app wiring).  
  Optional legacy ingest: `agent_fleet/services/gateway/src/routers/telemetry.py`.
- **Fleet:**  
  `agent_fleet/services/fleet_server/src/events.py`,  
  `agent_fleet/services/fleet_server/src/service.py`,  
  `agent_fleet/services/fleet_server/src/executor/executor.py` (emit hooks).
- **Telemetry:**  
  `agent_fleet/services/telemetry/src/app.py`,  
  `agent_fleet/services/telemetry/src/routers/ingest.py`,  
  `agent_fleet/services/telemetry/src/publishing.py`,  
  `agent_fleet/services/telemetry/src/heartbeat_store.py`.
- **Agents:**  
  `agent_fleet/agents/physical/*/server.py` (heartbeat URL / interval).
