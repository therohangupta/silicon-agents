# eda (framework adapters)

This **nested** package is the EDA / verification **tool adapter** layer. It is intentionally **not** the same as the parent **`domains.eda`** domain package name — import adapters as **`domains.eda.eda`**.
...
```python
from domains.eda.eda import get_eda_adapter
...
| [`../../../agents/backend/`](../../../agents/backend/) | Catalog agents that declare EDA operations in `config.yaml` |
