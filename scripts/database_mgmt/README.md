...
- Invoked from **repo root** so **`docker compose`** resolves **`docker-compose.yml`**
- Write access to **`db_backups/`** (created on demand for backups)

## Scripts

### backup_db.sh

1. Ensures **`db_backups/`** exists under **repo root**
...
From **repo root** with platform Compose up:
